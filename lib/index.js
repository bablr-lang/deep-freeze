...
let validate = preimplemented
  ? null
  : (value, shouldFreeze = false) => {
      if (!isObjecty(value)) return 2;
      let cached;
      ...
      for (let name of getOwnPropertyNames(obj)) {
        ...
        status &= (desc.get || desc.set ? 1 : 2) & validate(desc.value, shouldFreeze);
      }
      ...
      if (!shouldFreeze && (status === 0 || !isFrozen(obj))) return 0;
